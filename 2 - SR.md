...
| UFR-CON-106 | `image --in <src> --out <dst> [--st\|--msa\|--dir]` shall use explicit source and destination arguments; positional-only `image --msa path.st` shall print a deprecation warning and refuse. Without `--out`, defaults to `./disk.st`, `./disk.msa`, or `./disk/`. (P58) | UC24G |
...
| UFR-TRC-012 | `trace level <all\|info\|error\|todo>` shall set a display filter on the GUI trace window; lines below the filter level shall be hidden without being removed from the ring buffer or the log file. Levels (inclusive upward): `todo`=LOG_TODO only; `error`=ERROR+TODO; `info`=INFO+ERROR+TODO; `all`=TRACE+INFO+ERROR+TODO (replaces deprecated `trace`). (P28, ADAPTED P57 UC24G) | UC5, UC24G |
...
| UFR-DIR-015 | CTRL+SPACE shall toggle multi-selection of the currently focused **file** entry (not directories). The multi-selected set is displayed with a purple background (`g_dir_clrMultiSel`), distinct from the green last-committed indicator (P11) and the blue cursor highlight. The set holds up to `DIR_MULTI_SEL_MAX` paths. Activating CTRL+SPACE shall **clear** the single (green) selection (`szLastSelected`); activating plain SPACE shall **clear** the multi-selection set. The two modes are mutually exclusive at all times. (P14, ADAPTED P60 UC24G) | UC18.2, UC24G |

### 1.5 File System Abstraction — `FIL` (UC6)

> **Note (DOC-02):** These entries are **derived/design requirements** (platform portability abstraction). They do not map to a user-visible business story; they pin the contract between `file.h` and all callers. Parent UFR: `—` (derived). Corresponding SW requirements in §2.10.
...
| UFR-EDT-007 | `edit backup [on\|off]` shall toggle automatic backup creation on open (enabled by default at startup): opening a text file shall create `<name>_YYYYMMDD_HHMMSS.bak` in the same directory; on close the backup shall be deleted if the file content is byte-for-byte identical to the backup, or kept if modified. Both CRLF and LF-only text files shall load with all lines intact. | ✓ UC8-bak | UC8 |
...
| UFR-MNT-004  | ESC or the window close button shall close the mount view. If the in-memory image is dirty (modified since open), a save dialog is offered (see UFR-MNT-008). The `umount` command was removed in UC24E (see UFR-CON-099 — REMOVED). | ✓ UC18.1, ADAPTED UC24E | UC18.1 |
...
> **Note:** These entries were listed in §1.1 (CON) through UC24F but they are **SW-level design constraints**, not user-visible behaviours. They describe internal module APIs and data structures. They have been moved here per DOC-01 review (2026-06-10). Their IDs are preserved unchanged to maintain TC.md traceability.

| ID          | Internal Design Constraint                                                                                                                        | UC      |
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------|---------|
| UFR-CON-100 | `image_st_mkdir(ptImg, szName)` shall create a FAT12 subdirectory entry in the root directory of an in-memory image, allocating one cluster initialised with `.` and `..` entries. | UC24F |
| UFR-CON-101 | `image_st_list_dir(ptImg, szDirName, ...)` shall enumerate entries in a named subdirectory; `NULL`/`""` shall behave identically to `image_st_list_root()`. | UC24F |
| UFR-CON-102 | `image_st_write_file_in_dir(ptImg, szDirName, szFileName, pData, uiSize)` shall write a file into a named subdirectory's cluster chain, finding a free slot (0x00 or 0xE5) in the cluster data. | UC24F |
| UFR-CON-103 | `mount_view_t` shall include `szCurDir[IST_NAME_MAX]`, `aszNavStack[8][IST_NAME_MAX]`, and `iNavDepth` to track in-image directory navigation state. | UC24F |
...
| REQ-CON-035  | `line_cmd_dir()` shall accept a `--select <path>` flag: validate the path with `file_stat()`; on success call `line_set_selected(ptCtx, szSelPath)` + `line_update_console_title()` and return `ST_NO_ERROR` without opening the dir GUI view; on missing path arg or non-existent path print error and return `ST_NO_ERROR`. | UFR-CON-094 | ✓ UC24E | `line_cmd_dir()` |
| REQ-CON-036  | `line_cmd_script()` (`CMD_SCRIPT`, alias `r`) shall require exactly one argument; with no argument it shall print a usage warning and return `ST_NO_ERROR`; with one argument it shall call `line_exec_script(ptCtx, argv[1])` and propagate its return via `line_print_error` on failure. `line_exec_script(ptCtx, szPath)` shall open `szPath`, execute each non-comment non-blank line via `line_dispatch()`, and return `ST_ERROR` if the file cannot be opened. | UFR-CON-095 | ✓ UC24E | `line_cmd_script()`, `line_exec_script()` |
| REQ-CON-037  | `CON_KEY_CTRL_O` (0x0F) shall be defined in `console.h`; in `line_read_rich()` it shall dispatch `"mount"` via `line_shortcut()`, replacing the removed `CON_KEY_CTRL_U`. | UFR-CON-096 | ✓ UC24E | `line_read_rich()` |
| REQ-CON-038  | `line_cmd_edit()` shall parse `-h`/`--hex` flags before the filename; when present, `bForceHex = ST_TRUE` shall bypass extension detection and call `edit_hex_open()` unconditionally; without the flag, extension routing is unchanged. | UFR-CON-097 | ✓ UC24E | `line_cmd_edit()` |
| REQ-CON-039  | `line_cmd_image()` with `--dir [folder]` shall: (1) compute the destination directory (`folder` if given, else auto-number `disk`/`disk2`/… in cwd); (2) if a mount view is open, call `mount_save_image(MOUNT_SAVE_DIR, dest)`; (3) else if a `.st`/`.msa` file is selected or specified, open it via `mount_view_open()` + `mount_save_image(MOUNT_SAVE_DIR, dest)` + `mount_view_close()`; (4) else print a warning. | UFR-CON-098 | ✓ UC24E | `line_cmd_image()` |
| REQ-CON-040  | `CMD_UMOUNT` shall not exist in `cmd_id_t`; `line_cmd_umount()` shall not exist; the string `"umount"` shall not appear in `g_line_aCmds[]`; `line_complete_cmd_query("umount", ...)` shall return 0. | UFR-CON-099 (REMOVED) | ✓ UC24E | `line.h`, `line.c` |
| REQ-CON-041  | `line_cmd_trace()` `level` sub-command shall accept `all\|info\|error\|todo`; `all` maps to `LOG_LEVEL_TRACE` (show everything); `info` maps to `LOG_LEVEL_INFO`; `error` maps to `LOG_LEVEL_ERROR`; `todo` maps to `LOG_LEVEL_TODO`. The canonical display string for `LOG_LEVEL_TRACE` shall be `"all"`. Unknown level strings shall emit a warning listing all valid values. (P57) Impl: `line_cmd_trace()` | UFR-TRC-012 | ✓ UC24G | `line.c` |
| REQ-CON-042  | `line_cmd_image()` shall accept `--in <src>` (explicit source path: directory or `.st`/`.msa` file) and `--out <dest>` (explicit output path) flags. When `--in` is absent, source priority is: open mount view → `line_get_selected()`. When `--out` is absent, the destination is auto-named (`disk.st`, `disk.msa`, or `disk/`). A positional path argument without `--in`/`--out` shall emit a deprecation warning and be rejected. (P58) Impl: `line_cmd_image()`, `line_is_disk_path()` | UFR-CON-106 | ✓ UC24G | `line.c` |
...
| REQ-DIR-028  | **Exclusive selection (P60 UC24G):** when CTRL+SPACE activates multi-selection and `szLastSelected` is non-empty, `szLastSelected` shall be cleared and `line_set_selected()` called with `""` before toggling the multi-sel entry. Conversely, when plain SPACE activates single-selection and `iMultiSelCount > 0`, `aszMultiSel[]` and `iMultiSelCount` shall be zeroed before writing `szLastSelected`. The two modes are mutually exclusive at all times; `dir_render()` shall never simultaneously draw both green and purple backgrounds on the same row. | UFR-DIR-015 | ✓ UC24G | UC24G |
...
| REQ-EDT-014  | `etxt_load()` shall correctly handle both CRLF (`\r\n`) and LF-only (`\n`) files: every line break shall produce a separate entry in `aszLines[]`; the end-of-buffer sentinel (`\0`) shall be detected *before* overwriting the current byte, so that the LF→NUL substitution never prematurely terminates line parsing. (Refines REQ-EDT-004; fixes BUG-10.) | UFR-EDT-001 | ✓ UC8-bak | UC8 |
| REQ-EDT-015  | When `edit_txt_get_backup()` returns `ST_TRUE`, `edit_txt_open()` shall create `<szPath>_YYYYMMDD_HHMMSS.bak` before loading the file content; failure to create the backup shall log a warning and not prevent the open from succeeding. | UFR-EDT-007 | ✓ UC8-bak | UC8 |
| REQ-EDT-016  | On `GUI_EVT_CLOSE`, if `szBackupPath` is non-empty and the current file content is byte-for-byte identical to the backup, the backup file shall be deleted; if the contents differ, the backup shall be retained. | UFR-EDT-007 | ✓ UC8-bak | UC8 |
| REQ-EDT-017  | `edit_txt_set_backup(bEnabled)` shall set a module-level flag; `edit_txt_get_backup()` shall return the current value of that flag. The flag defaults to `ST_TRUE` at application startup. | UFR-EDT-007 | ✓ UC8-bak | UC8 |
...
| REQ-IST-030  | `image_st_mkdir(NULL, *)` or `image_st_mkdir(*, NULL)` shall return `ST_ERROR`. The name shall be validated as 8.3 (base 1–8 chars, extension 0–3 chars); an invalid name shall return `ST_ERROR`. A duplicate name (case-insensitive match in root dir) shall return `ST_ERROR`. The root directory entry shall have `IST_ATTR_SUBDIR` set, and the allocated cluster shall be initialised with `.` (cluster=self) and `..` (cluster=0) entries; remaining slots shall be zeroed. | UFR-CON-100 | ✓ UC24F | `image_st_mkdir()` |
| REQ-IST-031  | `image_st_list_dir(NULL,*,*,*,*)` or `image_st_list_dir(*,*,NULL,*,*)` or `image_st_list_dir(*,*,*,0,*)` or `image_st_list_dir(*,*,*,*,NULL)` shall return `ST_ERROR`. With `szDirName` NULL or `""`, the function shall delegate to `image_st_list_root()`. Otherwise the named entry must exist and have `IST_ATTR_SUBDIR`; absence or a non-directory match shall return `ST_ERROR`. Dot entries, volume labels and deleted entries shall be skipped. Enumeration shall stop at the first 0x00 entry. | UFR-CON-101 | ✓ UC24F | `image_st_list_dir()` |
| REQ-IST-032  | `image_st_write_file_in_dir()` shall return `ST_ERROR` if `ptImg`, `szDirName`, or `szFileName` is NULL, if `pData` is NULL and `uiSize > 0`, if either name is not valid 8.3, or if the named directory is not found. It shall walk the subdirectory cluster chain to find a free slot (first byte 0x00 or 0xE5) before allocating data clusters; a full cluster with no free slot shall return `ST_ERROR` (single-cluster subdirectories only in UC24F). | UFR-CON-102 | ✓ UC24F | `image_st_write_file_in_dir()` |
| REQ-IST-033  | `mount_view_t.szCurDir` shall be an empty string at `mount_view_open()` time; `iNavDepth` shall be 0. ENTER on a SUBDIR entry shall copy `szName` to `aszNavStack[iNavDepth]`, increment `iNavDepth`, copy `szName` to `szCurDir`, and call `mount_refresh()`. LEFT/BACKSPACE shall decrement `iNavDepth`, restore `szCurDir` from the stack (or clear it for root), and call `mount_refresh()`. Navigation shall be a no-op when `iNavDepth == 0` for LEFT/BACKSPACE. | UFR-CON-103, UFR-CON-104 | ✓ UC24F | `mount_view_t`, `mount_handle_key()` |
| REQ-IST-034  | `mount_dir_cb()` shall detect local subdirectories (`ptStat->bIsDir == ST_TRUE`); for each it shall call `image_st_mkdir()` on the subdirectory name, then enumerate its files via `file_list_dir()` and call `image_st_write_file_in_dir()` for each. Errors (mkdir, write_in_dir) shall be logged and continue the loop non-fatally. Nesting depth is limited to 1 level. | UFR-CON-105 | ✓ UC24F | `mount_dir_cb()` |
| REQ-IST-035  | `mount_save_image(MOUNT_SAVE_DIR, ...)` shall, for each SUBDIR entry in the snapshot, create a local subdirectory via `file_mkdir()`, call `image_st_list_dir()`, and extract each sub-entry via `image_st_read_file()` + `file_write()`. Root files are extracted as before. Errors during subdir extraction shall be logged and continue non-fatally. | UFR-CON-105 | ✓ UC24F | `mount_save_image()` |
...
| REQ-MNT-013  | The properties panel shall display BPB geometry fields in read-only mode: heads (`@0x1A` LE16), sectors/track (`@0x18` LE16), tracks (derived: `TS@0x13 / (SPT × Heads)`), root dir capacity RDE (`@0x11` LE16). The `/RDE` count shall be removed from the left panel header (kept in right panel as `Files: N / RDE`). The "Bootable: Yes/No" indicator uses `mount_check_bootable()` (WD1772 checksum). For `MOUNT_SRC_DIR` sources, all geometry fields shall display `"—"` (no bootsector available). (P34, P36, P37, BUG-05, BUG-06) | UFR-MNT-005, UFR-MNT-006 | ✓ UC18.2, ADAPTED UC24G | UC24G |
...
| REQ-MNT-025  | `mount_view_t` shall include `iNotImported` (int, initialised to 0). `mount_dir_cb()` shall increment `iNotImported` for each file skipped due to 8.3 name constraint or disk-full condition. `mount_render_panel()` shall display a `"Skipped: N (8.3 limit)"` row in orange/dirty colour when `iNotImported > 0`. (BUG-04) | UFR-MNT-003 | ✓ UC24G | UC24G |
...
### §2.27 DEVPAC3 Assembler — ALU + Control Flow (UC30C)

> Parent UFR: UFR-ASM-005 (§1.12)
...
| REQ-AS-028    | BRA/BSR/Bcc (14 conditions HI/LS/CC/CS/NE/EQ/VC/VS/PL/MI/GE/LT/GT/LE; aliases BHS=BCC, BLO=BCS) shall assemble in long form (4 bytes): opword `0x6000\|(cc<<8)` + 16-bit signed displacement = target−(opword_addr+2). Pass 1 advances PC by 4 without displacement resolution. Pass 2 reports ST_ERROR if label is undefined or displacement is outside [−32768, 32767]. Impl: `as_encode_branch()`, `g_as_aBcc[]`, `as_bcc_code()` in `as.c` | UFR-ASM-005 | ✓ UC30C | UC30C |

**Function table — `as.h` (UC30C additions):**

| Function | REQ | Impl. |
|---|---|---|
| `as_encode_alu_binary()` (internal) | REQ-AS-021 | `as.c` |
| `as_encode_cmp()` (internal) | REQ-AS-022 | `as.c` |
| `as_encode_eor()` (internal) | REQ-AS-023 | `as.c` |
| `as_encode_imm_alu()` (internal) | REQ-AS-024 | `as.c` |
| `as_encode_addq_subq()` (internal) | REQ-AS-025 | `as.c` |
| `as_encode_unary()` (internal) | REQ-AS-026 | `as.c` |
| `as_encode_ext()` (internal) | REQ-AS-026 | `as.c` |
| `as_encode_branch()` (internal) | REQ-AS-028 | `as.c` |
| `as_bcc_code()` (internal) | REQ-AS-028 | `as.c` |
| `as_encode_implied()` (internal) | REQ-AS-027 | `as.c` |
| `as_encode_stop()` (internal) | REQ-AS-027 | `as.c` |
| `as_encode_trap()` (internal) | REQ-AS-027 | `as.c` |
