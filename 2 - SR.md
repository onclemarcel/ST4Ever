...
| UFR-CON-099 | The `umount` command and its `CTRL+U` shortcut shall be removed; disk unmounting is replaced by the GUI close button (ESC) and `image --dir/--st/--msa` for headless saves. | UC24E |
...
| REQ-CON-040  | `CMD_UMOUNT` shall not exist in `cmd_id_t`; `line_cmd_umount()` shall not exist; the string `"umount"` shall not appear in `g_line_aCmds[]`; `line_complete_cmd_query("umount", ...)` shall return 0. | UFR-CON-099 | ✓ UC24E | `line.h`, `line.c` |
