...
**R24.2 — Les helpers internes doivent être tagués eux aussi ; « déjà couvert par l'appelant » ou rationale d'inaccessibilité** *(établie 2026-07-20, appliquée à `dir.c` [DIR]41-77 / `use_case_04.c`)*

R24 §2 autorisait à ne pas tagger « les fonctions internes < 5 lignes sans effet de bord ». En pratique, un helper statique appelé par un bloc déjà taggué peut lui-même contenir un GUARD, une branche ou un comportement caché/inattendu (troncature silencieuse, sentinelle jamais atteinte, ordre de scan implicite…) qui mérite d'être identifié et documenté même s'il n'est pas testé directement. R24.2 remplace la dispense de tagging par une obligation de **catégorisation complète** : chaque bloc fonctionnel d'un `.c` — y compris à l'intérieur des helpers statiques internes — reçoit un tag `[MODULE]N`, puis tombe dans l'une des trois catégories suivantes.

**Catégorie 1 — Couvert par le test de l'appelant.** Le comportement du helper est exercé indirectement chaque fois que l'appelant taggué est testé (pas besoin d'un nouveau test dédié). Dans le `.c` source, faire suivre le tag d'un commentaire `This helper is currently called by: - Xxx() ([MODULE]M) - tested when <condition>` recensant chaque appelant et le test/scénario qui l'exerce (exemples : `[DIR]41`/`dir_node_create`, `[DIR]54`/`dir_reexpand_path`, `[DIR]62`/`dir_navigate_up` dans `dir.c`). Dans le `use_case_NN.c`, recopier le tag inline juste en amont du bloc `INTENT[...]` de ce scénario **et** l'ajouter à la liste `Code Coverage:` de la fonction de test correspondante (règle R24 §3 inchangée) — même si le tag ne fait l'objet d'aucune nouvelle assertion dédiée.

**Catégorie 2 — Test dédié ajouté.** Le comportement est réellement testable mais n'était pas encore exercé (ex : ordre de tri d'un scan à deux passes, contenu exact d'un préfixe ASCII, mise à jour du titre de fenêtre, auto-scroll clavier, PAGE_UP/PAGE_DOWN). Ajouter le test dans l'un des groupes existants du `use_case_NN.c` (pas de nouveau groupe sauf nécessité), avec son propre `TC-xxx-NNN`/`INTENT[...]`. Les nouveaux TC peuvent être numérotés à la suite du dernier TC existant du module (`TC-DIR-093...`) même si leur position physique dans le fichier n'est pas strictement croissante avec ce numéro — R23 n'exige que l'unicité globale des identifiants, pas un ordre physique strict ; ne pas renuméroter en cascade tout le fichier pour préserver un ordre cosmétique quand le risque de régression dépasse le bénéfice.

**Catégorie 3 — Rationale d'inaccessibilité.** Le bloc est un GUARD défensif ou une branche qu'aucun test de ce binaire ne peut atteindre, et ça n'est pas un oubli mais un fait vérifiable par inspection des appelants. Cas typiques déjà rencontrés :
- **Garde toujours vraie côté appelant** : chaque site d'appel du helper garantit déjà la précondition (ex : `[DIR]48` `dir_node_load_children(NULL, ...)` — aucun appelant ne passe jamais `NULL`).
- **Sentinelle morte** : une valeur défensive n'est plus jamais produite par le code actuel (ex : `[DIR]64/67/68`, `iSelectedFlat == -2` — `dir_open()` initialise toujours à `-1`, aucun chemin n'assigne `-2`).
- **Précondition garantie par l'init** : un champ est initialisé à une valeur qui rend la garde inatteignable en pratique (ex : `[DIR]60/66/73/74`, `iCellH <= 0` — toujours initialisé à une valeur positive avant tout appel possible).
- **Frontière OS/fault-injection** : atteindre le bloc exigerait de sortir du dépôt (ex : `[DIR]58`, remonter jusqu'à la racine réelle du disque) ou de simuler un échec d'un backend externe sans hook de fault-injection disponible (ex : `[DIR]65`, échec de `renderer_create()`).

Dans ce cas, **ne pas** recopier le tag dans un `Code Coverage:` header (le check `[3]` de `check_tags.py` échouerait, car aucune copie inline/INTENT n'existerait). Documenter plutôt chaque tag de cette catégorie dans une section dédiée du `use_case_NN.c`, proche de `main()`, intitulée par convention `Untestable tag rationale (R24.2, CLAUDE.md §5 R24)` : lister le tag, sa raison précise (avec preuve — quel appelant garantit quoi), et grouper les tags qui partagent la même rationale plutôt que de répéter le même paragraphe. Le check `[2]` de `check_tags.py --module <MODULE>` reste `MISSING` pour ces tags — c'est attendu et informationnel (n'affecte jamais `--strict`) ; la section rationale est la preuve que le gap a été analysé, pas ignoré.

**Séquence pour un groupe désigné par Tonton Marcel** (extension de R25) :
1. Identifier chaque helper statique interne appelé par le bloc déjà taggué, même les plus courts.
2. Pour chacun, inspecter *tous* ses sites d'appel dans le `.c` pour déterminer si un argument invalide/une branche peut réellement survenir en pratique.
3. Attribuer un tag `[MODULE]N` (numérotation continue à la suite du dernier tag du module) à chaque GUARD/branche/bloc significatif, y compris ceux jugés inatteignables.
4. Classer chaque nouveau tag en Catégorie 1, 2 ou 3 et appliquer le traitement correspondant ci-dessus.
5. `python3 tools/check_tags.py --module <MODULE> --strict` : 0 erreur attendu ; le rapport `[2]` doit lister uniquement les tags de Catégorie 3 comme `MISSING`, jamais un tag de Catégorie 1/2 non câblé par oubli.
6. `make tests` : 0 warning, 0 failure avant de rendre la main.
