...
| UC4 | console | Line editor riche : historique ↑↓, Home/End, tab-completion, ghost-text ; prompt contextuel toggleable (trace/disk/fichier) ; `colors on/off` ; historique persistant `~/.st4ever_history` ; `--script file` batch mode ; `make manual` + macro `TEST_MANUAL` ; **UX dir** : ESC ferme vue (P9), flèches ←/→ expand/collapse (P12), ESPACE=sélection pure sans expand (P13), masquer fichiers cachés `.*` par défaut / `dir -a` pour tout voir (P15), focus auto fenêtre à l'ouverture (P16) | navigation + complétion + prompt état |
...
**P9 — ESC ferme la vue dir** → **ACCEPTÉ — UC4**

`gui_request_close(hWnd)` non-bloquant (poste WM_CLOSE sans `thread_join`) dans `gui.c` + `win_gui.c`. Groupé avec les améliorations threading de UC4. Applicable à toutes les vues futures.
...
**P12 — Flèches ←/→ pour expand/collapse** → **ACCEPTÉ — UC4**

Standard universel (VS Code, Explorer). Flèche droite = expand (ou descendre dans premier enfant si déjà ouvert), flèche gauche = collapse (ou remonter au parent si fermé). ~10 lignes dans `dir_handle_key()`.

**P13 — ESPACE = sélection pure, sans expand** → **ACCEPTÉ — UC4**

ENTER = action (navigate/expand), ESPACE = sélection pure. Cohérent avec Explorateur Windows. Trivial : 1 ligne dans `dir_handle_key()`.
...
**P15 — Masquer fichiers cachés `.*` par défaut** → **ACCEPTÉ — UC4**

Filtre dans `dir_node_load_children()` + option `dir -a` pour tout voir. Coût faible, pertinent pour le contexte ST4Ever (navigation fichiers ATARI ST).

**P16 — Focus auto fenêtre dir à l'ouverture** → **ACCEPTÉ — UC4**

`SetForegroundWindow()` + `SetFocus()` après `ShowWindow()` dans `win_gui.c`. Rend cohérent le flow "dir → flèches → ESPACE → ESC" sans alt-tab. Coût trivial, impact UX fort.
