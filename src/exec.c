 ...
 * ------------------------------------------------------------------ */
 ...
 static exec_context_t g_exec_ptCtx = {.ulMagic = 0xCAFEDECA,
                                       .eObject = ST_EXEC_CTX};
...
st_u64_t exec_init(st_u64_t ulSTMachineCtx)
{
    ...
    LOG_TRACE("Init exec module - attach %p ST Machine",
                (void*)ulSTMachineCtx);
    ...
    g_exec_ptCtx.bIsMachineOn = ptMachine->bPoweredOn;
    g_exec_ptCtx.bInitOK      = ST_TRUE;

    /* -- [EXEC]3. Init returns context sructure -- */
    LOG_INFO("execution engine initialised");
    return (st_u64_t)&g_exec_ptCtx;
}

st_error_t exec_shutdown(void)
{
    LOG_TRACE("bOpen=%d", g_exec_ptCtx.bOpen);
...
st_error_t exec_open(const char *szProgName, st_u32_t uiLoadAddr)
{
    ...
    memset(&(g_exec_ptCtx.tState), 0, sizeof(g_exec_ptCtx.tState));
    memset(&(g_exec_ptCtx.tCpu),   0, sizeof(g_exec_ptCtx.tCpu));

    eResult = platform_mutex_create(&(g_exec_ptCtx.tState.ptMutex));
    ...
    g_exec_ptCtx.tState.uiLoadAddr = uiLoadAddr;
    //g_exec_ptCtx.tState.ptMachine  = g_exec_ptMachine;
    if (szProgName != NULL)
    {
        strncpy(g_exec_ptCtx.tState.szProgName, szProgName, ST_MAX_PATH - 1);
    }
    ...
    platform_mutex_lock(g_exec_ptCtx.tState.ptMutex);
    exec_refresh_disasm(&g_exec_ptCtx.tState);
    platform_mutex_unlock(g_exec_ptCtx.tState.ptMutex);

    /* Set bOpen early so exec_get_state() works inside view opens.
     * Cleared on any error path below. */
    g_exec_ptCtx.bOpen = ST_TRUE;
    ...
    if (eResult != ST_NO_ERROR || ptMonView == NULL)
    {
        ...
        g_exec_ptCtx.bOpen = ST_FALSE;
        platform_mutex_destroy(&g_exec_ptCtx.tState.ptMutex);
        return ST_ERROR;
    }
    g_exec_ptCtx.tState.hMonWnd = ptMonView->hWnd;
    ...
    g_exec_ptCtx.tState.hCpuWnd = ptCpuView->hWnd;
    ...
    g_exec_ptCtx.tState.hMemWnd = ptMemView->hWnd;
    ...
    g_exec_ptCtx.tState.hAsmWnd = ptAsmView->hWnd;
    ...
    g_exec_ptCtx.tState.hScrWnd = ptScrView->hWnd;

    /* Start execution thread */
    eResult = platform_thread_create(&g_exec_ptCtx.ptThread,
                                     exec_thread_fn,
                                     &g_exec_ptCtx.tState);
    if (eResult != ST_NO_ERROR)
    {
    ...
    LOG_INFO("execution session opened: %s at $%06X",
             g_exec_ptCtx.tState.szProgName, uiLoadAddr);
    return ST_NO_ERROR;
}
...
st_error_t exec_close(void)
{
    ...
    platform_mutex_lock(g_exec_ptCtx.tState.ptMutex);
    g_exec_ptCtx.tState.bQuitReq = ST_TRUE;
    hMonWnd = g_exec_ptCtx.tState.hMonWnd;
    hCpuWnd = g_exec_ptCtx.tState.hCpuWnd;
    hMemWnd = g_exec_ptCtx.tState.hMemWnd;
    hAsmWnd = g_exec_ptCtx.tState.hAsmWnd;
    hScrWnd = g_exec_ptCtx.tState.hScrWnd;
    g_exec_ptCtx.tState.hMonWnd  = NULL;
    g_exec_ptCtx.tState.hCpuWnd  = NULL;
    g_exec_ptCtx.tState.hMemWnd  = NULL;
    g_exec_ptCtx.tState.hAsmWnd  = NULL;
    g_exec_ptCtx.tState.hScrWnd  = NULL;
    platform_mutex_unlock(g_exec_ptCtx.tState.ptMutex);

    /* Join execution thread */
    if (g_exec_ptCtx.ptThread != NULL)
    {
        platform_thread_join(g_exec_ptCtx.ptThread);
        platform_thread_destroy(&g_exec_ptCtx.ptThread);
    }
    ...
    /* Release mutex */
    if (g_exec_ptCtx.tState.ptMutex != NULL)
    {
        platform_mutex_destroy(&g_exec_ptCtx.tState.ptMutex);
    }
...
st_error_t exec_step_request(void)
{
    ...
    platform_mutex_lock(g_exec_ptCtx.tState.ptMutex);
    if (g_exec_ptCtx.tState.eRunState != EXEC_RUN_HALTED)
    {
        g_exec_ptCtx.tState.bStepReq = ST_TRUE;
    }
    platform_mutex_unlock(g_exec_ptCtx.tState.ptMutex);
    return ST_NO_ERROR;
}
...
st_error_t exec_run_request(void)
{
    ...
    if (g_exec_ptCtx.tState.eRunState != EXEC_RUN_HALTED)
    {
        g_exec_ptCtx.tState.bRunReq = ST_TRUE;
    }
    platform_mutex_unlock(g_exec_ptCtx.tState.ptMutex);
    return ST_NO_ERROR;
}
...
st_error_t exec_pause_request(void)
{
    ...
    platform_mutex_lock(g_exec_ptCtx.tState.ptMutex);
    g_exec_ptCtx.tState.bPauseReq = ST_TRUE;
    platform_mutex_unlock(g_exec_ptCtx.tState.ptMutex);
    return ST_NO_ERROR;
}
...
st_error_t exec_stop_request(void)
{
    ...
    platform_mutex_lock(g_exec_ptCtx.tState.ptMutex);
    g_exec_ptCtx.tState.bStopReq = ST_TRUE;
    platform_mutex_unlock(g_exec_ptCtx.tState.ptMutex);
    return ST_NO_ERROR;
}
...
st_error_t exec_bp_toggle(st_u32_t uiAddr)
{
    ...
    iIdx = exec_bp_find(&g_exec_ptCtx.tState, uiAddr);
    if (iIdx >= 0)
    {
        /* Remove: shift remaining entries down */
        int j;
        for (j = iIdx; j < g_exec_ptCtx.tState.iBPCount - 1; j++)
        {
            g_exec_ptCtx.tState.auBP[j] = g_exec_ptCtx.tState.auBP[j + 1];
        }
        g_exec_ptCtx.tState.iBPCount--;
        LOG_INFO("breakpoint removed at $%06X", uiAddr);
    }
    else if (g_exec_ptCtx.tState.iBPCount < EXEC_BP_MAX)
    {
        g_exec_ptCtx.tState.auBP[g_exec_ptCtx.tState.iBPCount] = uiAddr;
        g_exec_ptCtx.tState.iBPCount++;
...
st_error_t exec_bp_clear(void)
{
    ...
    platform_mutex_lock(g_exec_ptCtx.tState.ptMutex);
    g_exec_ptCtx.tState.iBPCount = 0;
    platform_mutex_unlock(g_exec_ptCtx.tState.ptMutex);
